...
Processor.prototype.handleMasterMessage = function (data) {
    ...
    case 'load': //['load', job, options, worker_id]
        ...
        load(data[1], function (err, job_name, job_obj) {
            if (err) {
                self.exit(err);
            } else {
                self.startJob(job_name, job_obj);
            }
        });
        break;
        ...
        instance.retry = function () {
            instance.finish(function () {
                ...
                if (job.options.retries !== false) {
                    ...
                    if (job.retry_hashes[input_hash] > job.options.retries) {
                        instance.fail_with('retry');
                        return;
                    }
                }
