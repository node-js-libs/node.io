...
exports.start = function(job, options, callback, capture_output) {
    ...
    var onload = function(job, job_obj) {
        ...
        p.init(function() {
            p.startJob(job, job_obj, oncomplete_callback);
        });
    }
    ...
    if (typeof job !== 'string') {
        ...
        var joblen = job.length;
        if (path.extname(job) === '.coffee') {
            
            var basename = path.basename(job, '.coffee');
            var compiled_filename = cwd + '/' + basename + '_compiled.js';
            ...
            if (!process.env._CHILD_ID_) {
                ...
                exec('coffee -p -c "'+job+'"', {cwd:cwd}, function(err, stdout, stderr) {
                    if (err || stderr) {
                        ...
                        fs.writeFileSync(compiled_filename, stdout);
                        
                        var oldcallback = callback;
                        callback = function (err, output) {
                            fs.unlinkSync(compiled_filename);
                            oldcallback(err, output);
                        };
                        
                        job = compiled_filename;
