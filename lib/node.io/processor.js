...
exports.start = function (job, options, callback, capture_output) {
	...
	var proc = new Processor(options);
        
    //Callback for once we've loaded the job
    var onload = function (job, job_obj) {
        
        //If we're capturing output, we need to explicitly overwrite the output method
        var output;
        if (capture_output) {
            output = [];
        ...
        var oncomplete_callback = function (err) {
            callback(err, output);
        };
        ...
        proc.init(function () {
            ...
            //Start the job
            proc.startJob(job, job_obj, oncomplete_callback);
            
        });
    };
    
    //Load the job. Handle the case where `job` is a string (path) or an object (already require()'d)
    if (typeof job !== 'string') {
            
        //Give the job a unique identifier
        ...
        if (path.extname(job) === '.coffee') {
            ...
            if (!process.env._CHILD_ID_) {
                ...
                utils.compileCoffee(job, compiled_js, function(err) {
                    if (err) {
                        ...
                        //Remove the temp file once we're done with it
                        ...
                        //Remove the compiled file once the job is complete
                        var oldcallback = callback;
                        callback = function (err, output) {
                            removeTemp();
                            oldcallback(err, output);
                        };
                        
                        //Load the compiled JS file
                        onload(compiled_js, proc.loadJob(compiled_js));
                    }
                });
                
            } else {
                
                //If we're a child, the file is already compiled
                onload(compiled_js, proc.loadJob(compiled_js));
            ...
            //We're dealing with a plain old .js file
            onload(job, proc.loadJob(job));
            
        }
    }    
};

//TODO: start the processor as a slave. It will sit and wait until it receives instructions
exports.startSlave = function (options) {
	var proc = new Processor(options);
    isSlave = !process.env._CHILD_ID_;
	proc.init(function () {
...
Processor.prototype.init = function (next, spawn_children) {
    ...
    if (process.platform === 'cygwin' || (!fork && isMaster)) {
                
        //Note: node doesn't support FD passing in Windows so unable to fork
        if (next) {
            next();
        }
        ...
        //When using child processes, the master delegates input and does no actual work
        if (isMaster) {
            isWorker = false;    
        }
...
//This method acts as an inter-process message handler and router
Processor.prototype.handleMessage = function (data) {
    ...
    case 'err':            
        this.jobs[data.job].oncomplete(data.err);
        break;
...
//Wrap utils.status and omits status messages when using the `-s` switch or `silent` option
...
//Checks whether each worker has finished their work
Processor.prototype.workersComplete = function (job) {
    var j = this.jobs[job];
    for (var i = 0; i < worker_count; i++) {
        if (!j.worker_complete[i]) {
            return false;
        }
    }
    return true;
...
Processor.prototype.startJob = function (job, job_obj, oncomplete) {
    var j = this.jobs[job] = {};
    ...
    if (isMaster) {
        ...
        if (j.options.global_timeout) {
            j.global_timeout = setTimeout(function () {
                j.oncomplete('Operation timed out (> ' + j.options.global_timeout + 's)');
...
Processor.prototype.pullInput = function (job, forWorker) {
    ...
    j.ready_to_request_input = false;
...
Processor.prototype.jobComplete = function (job) {        
    ...
    if (isMaster) {
        ...
        //Remove the timeout if it was set
        if (j.global_timeout) {
            clearTimeout(j.global_timeout);
        }
                
        //Call `job.complete()` if it is defined
        ...
        if (j.options.benchmark) {
            ...
            var mb_read = Math.round(j.obj.getBytesRead() / (pow * pow) * 1000) / 1000;
            var mb_written = Math.round(j.obj.getBytesWritten() / (pow * pow) * 1000) / 1000;
        ...
        //Wait for output streams to drain before completion
        if (j.obj.output_streams) {
            var wait_for_drain = function () {
                
                var keep_waiting = false;
                for (var i in j.obj.output_streams) {
                    if (j.obj.output_streams.hasOwnProperty(i)) {
                        var stream = j.obj.output_streams[i];
...
Processor.prototype.instanceSpawn = function (job) {
    ...
    if (j.ready_to_request_input && j.input.length < (j.options.max * j.options.take)) {
        
        self.pullInput(job);
        
    } else if (!isMaster && !j.isComplete && j.input.length === 0) { 
    
        //We might be done, add another check on the nextTick
        process.nextTick(function () {
            if (j.input.length === 0 && !j.isComplete) {
                var output;
                if (j.output.length) {
                    output = j.output;
                    j.output = [];
...
Processor.prototype.instanceStart = function (job, input) {    
    ...
    //Create an instance with a subset of the methods defined in the job
    var instance = new JobClass(j.obj.options);
    ...
    //Allow job methods to add input using job.add()
    instance.add = function (add, dont_flatten) {
        self.addInput.apply(self, [job, add, dont_flatten]);
    };
    
    //Handle the re-adding of input on retry(), or fail if input exceeds the
    //maximum number of retries
    instance.retry_hook = function (input) {    
        if (j.options.retries !== false) {
            var input_hash = utils.crc32(JSON.stringify(input));
    ...
    //Bind a proxy if the `proxy` option is set
    var proxy = j.options.proxy;
    if (proxy instanceof Proxy) {
        proxy.proxify(instance);
    } else if (typeof proxy === 'function') {
        //Allow the user to specify a callback which returns a Proxy
        proxy().proxify(instance);        
    }
