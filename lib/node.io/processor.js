...
exports.start = function (job, options, callback, capture_output) {
    ...
    var onload = function (job_name, job_obj) {
        ...
        if (options.unpack && typeof job_obj.unpack === 'function') {
            processor.status('Unpacking the job', 'debug');
            try {
        ...
        processor.status('Running job "' + (which_job || job_name) + '"', 'debug');
...
Processor.prototype.init = function (callback) {
    ...
    if (isMaster && !fork) {
        ...
        this.status('Fork is currently unsupported on Cygwin (Node does not support FD passing in Windows)', 'error');
        return callback();
...
Processor.prototype.loadJob = function (job, callback) {
    ...
    if (typeof job === 'object') {
        ...
        if (!is_coffee) {
            ...
            try {
                callback(null, job, require(job));
            } catch (e) {
                this.status(JSON.stringify(e), 'debug');
                callback('Failed to load job "' + job + '". Please check that the job exists and compiles correctly.');
            }
            ...
            var compiled_js = basename + '_compiled.js';
            ...
            if (isMaster) {
                ...
                utils.compileCoffee(job, compiled_js, function(err) {
                    if (err) {
                        ...
                        callback(null, compiled_js, require(compiled_js));
                    }
                });
...
Processor.prototype.startJob = function (job_name, job_obj, oncomplete) {
    ...
    var id = utils.crc32(job_name) + Math.floor(Math.random() * 100000);
